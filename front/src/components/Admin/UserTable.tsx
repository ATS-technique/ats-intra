        ...
        if (selectedUser && action) {
            if (action === "activate") {
                ...
                try {
                    ...
                    UpdateUser.body.nameUpdated = UpdateUserState.nameUpdated === "" ? selectedUser.name : UpdateUserState.nameUpdated;
                    UpdateUser.body.mailUpdated = UpdateUserState.mailUpdated === "" ? selectedUser.mail : UpdateUserState.mailUpdated;
    ...
    return (
        <div className="mt-4 w-full">
            <div className="px-4 bg-white dark:bg-neutral-900  shadow-md rounded mx-4 my-2 flex ">
                <p className='w-2/12 py-4 text-center dark:text-neutral-400'><strong>Nom</strong></p>
                <p className='w-3/12 py-4 text-center dark:text-neutral-400'><strong>Mail</strong></p>
                <p className='w-3/12 py-4 text-center dark:text-neutral-400'><strong>Date de création</strong></p>
                <p className='w-1/12 py-4 text-center dark:text-neutral-400'><strong>État</strong></p>
                <p className='w-1/12 py-4 text-center dark:text-neutral-400'><strong>Modifier</strong></p>
                <p className='w-2/12 py-4 text-center dark:text-neutral-400'><strong>Mot de passe</strong></p>
            </div>
            {users.length > 0 ? (
                users.map((user) => (
                    <div key={user.id_user} className="px-4 bg-white dark:bg-neutral-900 shadow-md rounded mx-4 my-2 flex ">
                        ...
                        <p className='w-1/12 py-4 text-center dark:text-neutral-400'>
                            {user.is_active ? (
                                <FontAwesomeIcon icon={faUserCheck} className="text-green-500 hover:text-green-400 cursor-pointer" onClick={() => handleClick(user, 'deactivate')} />
                            ) : (
                                <FontAwesomeIcon icon={faUserSlash} className="text-red-500 hover:text-red-400 cursor-pointer" onClick={() => handleClick(user, 'activate')} />
                            )}
                        </p>
                        <p className='w-1/12 py-4 text-center'>
                            {user.is_active ? (
                                <FontAwesomeIcon icon={faUserPen} className="dark:text-neutral-400 dark:hover:text-neutral-300 hover:text-neutral-700 cursor-pointer" onClick={() => handleClick(user, 'edit')} />
                            ) : ""}
                        </p>
                        <p className='w-2/12 py-4 text-center dark:text-neutral-400'>
                            {user.is_active ? (
                                <FontAwesomeIcon icon={faLock} className="dark:text-neutral-400 dark:hover:text-neutral-300 hover:text-neutral-700 cursor-pointer" onClick={() => handleClick(user, 'editPassword')} />
                            ) : ""}
                        </p>
                    </div>
                ))
                                ...
                                <div className='px-24'>
                                    {fields.map((field) => (
                                        <Input
                                            ...
                                            placeholder={field.name === "mailUpdated" ? selectedUser.mail : (field.name === "nameUpdated" ? selectedUser.name : "")}
                                        />
                                    ))}
                                </div>
                        ...
                        {action === 'editPassword' ?
                            <>
                                ...
                                <div className="flex justify-end">
                                    ...
                                    <div className='mx-2'>
                                        <FormAction handleSubmit={handleSubmit} text="Confirmer" isDisabled={passwordMatch ? false : true} />
                                    </div>
                                </div>
